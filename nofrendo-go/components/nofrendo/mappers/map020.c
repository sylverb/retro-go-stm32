...
{
    fds_cpu_timer(nes_getptr()->cycles_per_line);
}
...
void fds_init(void)
{
    uint8 *disk_ptr = nes_getptr()->rominfo->rom;
    int rom_size = osd_getromdata(&disk_ptr);
    ...
    {
        fds.sides = rom_size / 65500;
        MESSAGE_INFO("FDS header absent. Sides = %d\n", fds.sides);
    }

    for (int i = 0; i < 8; i++)
    {
        if (i < fds.sides || rom_size > i * 65500)
